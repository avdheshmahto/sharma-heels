...
<?php
	...
	$exp=explode('^',$conidget);
	 $ID=$exp[0];
	 //$locname=$exp[1];
 $querycon=$this->db->query("select * from tbl_location where STATUS='A' and id='$ID'");	  
 $fetchlistcon=$querycon->row();
 $locname=$fetchlistcon->location_name;
?>  
...
<?php echo $fetchlistcon->location_name;?></th>
...
<button class="btn btn-sm modalEditContact" onclick="EditcontactPriceLoc('<?php echo $fetch_list->id;?>')" href='#editcontact' type="button" data-toggle="modal" data-backdrop='static' data-keyboard='false'>Update Price</button>
...
var xhttp = new XMLHttpRequest();
 ...
 xhttp.open("GET", "EditPriceMapLoc?ID="+v, false);
 xhttp.send();
...
function getdata()
		 ...
		 var product=product1+'^'+location_name;
		    ...
		    if(xobj)
			 ...
			 xobj.open("GET","getproductloc?con="+product,true);
