...
$due=$this->db->query("select inv_no,amount_due,payment_status from tbl_payment_gst_dtl_new where payment_hdr_id='$invid[$j]' and inv_no='".$inv_det->gst_inv_id."'");
$duesum=$this->db->query("select sum(payment) as amount from tbl_payment_gst_dtl_new where payment_hdr_id='$invid[$j]' and inv_no='".$inv_det->gst_inv_id."'");
...
foreach($due_re as $due_res)
	{
		...
		if($due_res->payment_status!='done')
		{
...
<th><input type="text" value="<?=$inv_det->invoice_no?>" readonly="true" name="invoice_number[]" class="form-control"/>
...
<a class="btn btn-secondary btn-sm" href="<?=base_url("Payment/manage_gst_payment");?>">Cancel</a>
<td></td>
<td></td>
</th>
</tr>
