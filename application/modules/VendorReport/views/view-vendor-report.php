...
<a class="page-title" style="padding: 0 0 0 425px;font-size: 20px;">VENDOR REPORT&nbsp;</a>
<div class="row">
<div class="col-lg-12">
...
<label class="col-sm-2 control-label" style="color: #fff;">Vendor Name</label> 
<div class="col-sm-3"> 
<select name="vendor_id" class="form-control ui fluid search dropdown location" id="vendor_id">
						<option value="">----Select ----</option>
					<?php 
						$sqlstock=$this->db->query("select * from tbl_stockpoint_and_vendor where type='Vendor'");
						foreach ($sqlstock->result() as $fetchStock){						
					?>					
    <option value="<?php echo $fetchStock->stockpid; ?>"><?php echo $fetchStock->stockpointname; ?></option>
	...
	<div class="row">
		<div class="col-sm-12">
			...
			<div class="dataTables_length" id="DataTables_Table_0_length">
				<label>Show
				<select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries" url="<?=base_url('VendorReport/vendorbyreport');?>" class="form-control input-sm">
...
<?php
	...
	if($search!='')
	{
		...
		{	
			...
			$log_queryy=$this->db->query("select * from tbl_product_stock_log where status='A' and vendor_id='$vendor_id'");
			$fetchlog=$log_queryy->row();
			$fetchid=$fetchlog->p_logid;
			$queryy.=" and p_logid = '$fetchid'";
		}
		...
		$queryy .=" ORDER BY history_update_id DESC limit $page,$per_page";
