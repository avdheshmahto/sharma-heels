...
?>
<form id="f1" name="f1" method="POST" action="insertgstinvoice" onSubmit="return checkKeyPressed(a)">
<!-- Main content -->
	<div class="main-content">
	<a class="page-title" style="padding: 0 0 0 450px;font-size: 20px;">GST INVOICE</a>
...
<th>*Customer Name</th>
<th>
<select  name="customer_id" id="customer_id" class="form-control" required onchange="customer_name_id();">
						<option value="">----Select ----</option>
<?php
...
<div style="width:100%; padding-left:0px; color:#000000; border:2px solid " id="m">
<table id="invoice" style="width:100%;  height:70%;" title="Invoice"  >
<tr>
...
<tbody>
<tr>
<th style="text-align:right;padding: 0 16px 0px 400px;">Sub Gst Amount</th>
<th>
<input type="text" class="form-control" name="sumsubgst" id="sumsubgst" style="text-align:center; border:none; background:#FFFFFF;" readonly="true"  />

</th>
</tr>
	<tr> 
<th style="text-align:right;padding: 0 16px 0px 400px;">Total</th>
<th>
<input type="text" class="form-control" name="total" id="total" style="text-align:center; border:none; background:#FFFFFF" readonly="true"  value=""/>
</th>

<th style="display:none">
<input type="hidden" class="form-control" name="sumtot" id="sumtot" style="text-align:center; border:none; background:#FFFFFF;" readonly="true"/>

</th>
<th style="display:none">
<input type="hidden" class="form-control" name="sumgst" id="sumgst" style="text-align:center; border:none; background:#FFFFFF;" readonly="true"  />
...
function autogst()
	{
		...
		//tgst=Number(amt)*Number(18/100);
		document.getElementById("gst").value=gst;
		//document.getElementById("sumsubgst").value=tgst;
	}
		  ...
		  { 
				...
				var pd=document.getElementById("prd").value;
		   	   var table = document.getElementById("invoice");
					...
					var gstto=document.getElementById("sumsubgst").value;
					
					var subgst=document.getElementById("sumgst").value;
					

					gstto=Number(gstto)+Number(gst);
					sum=Number(sum)+Number(amt);
					sumsubgst=Number(subgst)+(Number(amt)*(0.18));
					//var gsts=gsts+gst;
					document.getElementById("sumtot").value=sum;
					document.getElementById("sumgst").value=sumsubgst;
					document.getElementById("sumsubgst").value=gstto;
...
function totals()
	{
		...
		var sumgstsub=document.getElementById("sumgst").value;
		var sumgst=document.getElementById("sumsubgst").value;
		
		
		document.getElementById("total").value=sumtot;
		document.getElementById("gstamt").value=sumgstsub;
		document.getElementById("sumsubgst").value=sumgst;
		var grandtotal=Number(sumtot)+Number(sumgstsub);
...
function customer_name_id()
	{
		...
		$.ajax({
			type:"POST",
			url:"<?=base_url('GstInvoice/post_c_firm_name')?>",
			data:datas,
			success:function(data){
											$(".c_firm_name_list").html(data);
								  }
				});		  
	}
</script>
