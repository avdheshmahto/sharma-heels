...
<li><a aria-expanded="false" href="#statement" data-toggle="tab">Payment Received</a></li>
</ul>
...
if($_GET['id']!='')
	{
    $queryy="select * from tbl_invoice_payment where  contact_id='".$_GET['id']."' and  status='invoice' or  status='PaymentReceived' order by id asc ";
...
<th><?php if($line->status=='PaymentReceived'){?><?php echo $c-=$line->receive_billing_mount;?><?php }else {?><?php echo $c; } ?></th>
<th>
<?php if($line->status=='PaymentReceived'){ echo "Payment Received"; }else{ echo $line->status;}?>
</th>	
</tr>
<?php
