...
<li><a href="form-basic.html">Bapa Nagar</a></li> 
...
<select name="location_id" id="location_id"  class="form-control" onchange="contactcode()"  required <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?> >
					<?php 
					$sqllo=$this->db->query("select * from tbl_location where id='7'");
...
$invhdr=$this->db->query("select * from tbl_ordered_invoice_hdr_bapa where customer_id='$fetch_list->contact_id'");
...
<label class="col-sm-2 control-label">Phone No:</label> 
<div class="col-sm-4" id="regid"> 
 <input type="text" maxlength="10"  pattern="[0-9]{10}" title="Enter your  phone number" name="phone" value="<?php echo $fetch_list->phone; ?>" readonly=""  class="form-control">
</div>
</div>
...
<script>
    $('.modalEditcontact').click(function(){
	    ...
	    $.ajax({url:"updateContactBapa?ID="+ID,cache:true,success:function(result){
