...
<select name="location_id" id="location_id"  class="form-control" onchange="contactcode()"  required <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?> >
					<?php 
					$sqllo=$this->db->query("select * from tbl_location where id='4'");
...
$invhdr=$this->db->query("select * from tbl_ordered_invoice_hdr_seel where customer_id='$fetch_list->contact_id'");
...
<script>
    $('.modalEditcontact').click(function(){
	    ...
	    $.ajax({url:"updateContactSeel?ID="+ID,cache:true,success:function(result){
