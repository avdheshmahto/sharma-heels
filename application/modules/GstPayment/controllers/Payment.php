...
public function gst_payment_Nat_edit(){
	if($this->session->userdata('is_logged_in')){
		$id=$_GET['id'];
	$this->load->view('Payment/gst-payment-Nat-edit',$id);
	}
	else
	{
	redirect('index');
...
public function manage_gst_payment_det()
	{
		if($this->session->userdata('is_logged_in')){
		 ////Pagination start ///
	  $url   = site_url('/Payment/manage-gst-payment_det?');
	  $sgmnt = "4";
	  $showEntries =10;
      ...
      if($_GET['entries']!=""){
         ...
         $url   = site_url('/Payment/manage_gst_payment_det?entries='.$_GET['entries']);
      }
...
public function tbl_payment_gst_insert()
	{
		...
		$this->load->view('Payment/gst_payment_table',$data1);
	}
public function gst_payment_recieved_view()
	{
			if($this->session->userdata('is_logged_in'))
			{
				...
				$data['id']=$_GET['view'];
				$this->load->view("Payment/gst-payment-Nat-view",$data);
...
public function manage_gst_payment()
	...
	if($this->session->userdata('is_logged_in')){
		 ////Pagination start ///
		 $this->load->model('model_admin_login');
	  $url   = site_url('/Payment/manage-gst-payment?');
	  $sgmnt = "4";
	  $showEntries =10;
      $totalData  = $this->model_admin_login->count_alll($this->input->get());
	       if($_GET['entries']!=""){
         $showEntries = $_GET['entries'];
         $url   = site_url('/Payment/manage_gst_payment?entries='.$_GET['entries']);
      }
         $pagination = $this->ciPagination($url,$totalData,$sgmnt,$showEntries);
...
public function use_credits()
	...
	if($this->session->userdata('is_logged_in')){
		 ////Pagination start ///
		 $this->load->model('model_admin_login');
	  $url   = site_url('/Payment/use_credits?');
	  $sgmnt = "4";
	  $showEntries =10;
      $totalData  = $this->model_admin_login->count_all($this->input->get());
	       ...
	       if($_GET['entries']!=""){
         $showEntries = $_GET['entries'];
         $url   = site_url('/Payment/use_credits?entries='.$_GET['entries']);
      }
         $pagination = $this->ciPagination($url,$totalData,$sgmnt,$showEntries);
