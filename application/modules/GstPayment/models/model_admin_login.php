...
function count_all($get){
  $qry ="select count(*) as countval from tbl_gst_invoice_hdr where status= ?";
   ...
   return $query[0]['countval'];
	...
	function count_alll($get)
		{
	$qry ="select count(*) as countval from tbl_payment_gst_hdr_new where status=?";
	if($get['filtername']!="" || $get['filterdate']!="" ){
      if($get['filtername']!="")
         $qry .= " AND name LIKE '%".$get['filtername']."%'";
   ...
   $query=$this->db->query($qry,array(''))->result_array();
   return $query[0]['countval'];
}
	}
