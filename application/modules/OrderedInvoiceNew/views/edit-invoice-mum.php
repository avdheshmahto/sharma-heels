...
<tbody>
<?php
   $orderhdrquert=$this->db->query("select * from tbl_ordered_invoice_hdr_mum where ordered_invoiceid_mum='$ID'");
...
<?php 
				$rowi=1;
	$orderdtlquert=$this->db->query("select * from tbl_ordered_invoice_dtl_mum where ordered_invoiceid_mum='$ID'");
...
<?php } ?>
<td>
<input type="hidden" name="order_idd[]" value="<?php echo $fetch_list_orderdtl->ordered_invoiceid_dtl_mum; ?>" />
