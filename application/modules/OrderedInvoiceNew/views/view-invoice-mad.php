...
<tbody>
<?php
   $orderhdrquert=$this->db->query("select * from tbl_ordered_invoice_hdr_mad where ordered_invoiceid_mad='$ID'");
...
<?php 
				$rowi=1;
	$orderdtlquert=$this->db->query("select * from tbl_ordered_invoice_dtl_mad where ordered_invoiceid_mad='$ID'");
...
<input type="hidden" name="order_idd[]" value="<?php echo $fetch_list_orderdtl->ordered_invoiceid_dtl_mad; ?>" />
<?php

$itemQuery = $this -> db
           -> select('*')
           -> where('Product_id_mad',$fetch_list_orderdtl->item_id)
           -> get('tbl_product_stock_madipur');
