...
<tbody>
<?php
   $orderhdrquert=$this->db->query("select * from tbl_ordered_invoice_hdr_bapa where ordered_invoiceid_bapa='$ID'");
...
<?php 
				$rowi=1;
	$orderdtlquert=$this->db->query("select * from tbl_ordered_invoice_dtl_bapa where ordered_invoiceid_bapa='$ID'");
...
<input type="hidden" name="order_idd[]" value="<?php echo $fetch_list_orderdtl->ordered_invoiceid_dtl_bapa; ?>" />
<?php

$itemQuery = $this -> db
           -> select('*')
           -> where('Product_id_bapa',$fetch_list_orderdtl->item_id)
           -> get('tbl_product_stock_bapanagar');
...
<td><?php echo $fetch_list_orderdtl->total_price; ?></td>
</tr>
<?php $rowi++; } ?>
