...
<tbody>
<?php
   $orderhdrquert=$this->db->query("select * from tbl_ordered_invoice_hdr_seel where ordered_invoiceid_seel='$ID'");
...
<?php 
				$rowi=1;
	$orderdtlquert=$this->db->query("select * from tbl_ordered_invoice_dtl_seel where ordered_invoiceid_seel='$ID'");
...
<?php } ?>
<td>
<input type="hidden" name="order_idd[]" value="<?php echo $fetch_list_orderdtl->ordered_invoiceid_dtl_seel; ?>" />
...
<input type="text" name="total_qty[]" id="totalorid<?php echo $rowi; ?>" class="form-control" value="<?php echo $fetch_list_orderdtl->total_qty; ?>" readonly />
