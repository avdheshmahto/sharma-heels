...
if($typeid=='loc'){
			...
			$sqlorder=$this->db->query("select * from tbl_order_hdr where invoice_status='Pending' and store_id='$idd'");
				
	}
	if($typeid=='custmr'){
	
			$sqlorder=$this->db->query("select * from tbl_order_hdr where invoice_status='Pending' and customer_id='$idd'");
...
<button class="btn btn-sm modalEditcontact" href="#editcontact" type="button" data-toggle="modal" data-backdrop="static" data-keyboard="false" onclick="invoiceedit('<?php echo $fetch_list->order_id;?>')"><i>Add Invoice</i></button>			
