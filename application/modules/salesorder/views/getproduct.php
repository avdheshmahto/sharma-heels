...

    {
	  ...
	  alert("Your broweser doesnot support ajax");
		...
		//document.getElementById("spid").value=tid;
		  ...
		  function abc(pt,pr,tid,q,inq,u,cateid,itid,ty){
		  				//alert(itid);
						var pid=pt.split("^");
		  				var pids=pid[1];
...
<?php
 ...
 if($con1!="" )
 {
  ...
  $sel=$this->db->query("select * from tbl_product_stock where order_status='A' and Product_type='$Productctg_id' and productname like '%$con3%'");
...
<input type="text" id="ty<?php echo $id;?>"  class="prds" value="<?php echo $arr->productname;?>" name="<?php echo $id;?>"
 onFocus="abc(this.value,'<?php echo $sizedata; ?>',this.id,'<?php echo $qtyrr; ?>','<?php echo $orderedqtyinstock; ?>','<?php echo $categoryname; ?>','<?php echo $categoryid; ?>','<?php echo $arr->Product_id;?>','<?php echo $protypefetch->keyvalue; ?>')"
 onClick="abc(this.value,'<?php echo $sizedata; ?>',this.id,'<?php echo $qtyrr; ?>','<?php echo $orderedqtyinstock; ?>','<?php echo $categoryname; ?>','<?php echo $categoryid; ?>','<?php echo $arr->Product_id;?>','<?php echo $protypefetch->keyvalue; ?>')" tabindex="-1"  readonly >
